...
// .wrangler/tmp/bundle-TrhZkJ/checked-fetch.js
...
// .wrangler/tmp/pages-bTminO/functionsWorker-0.19429245638868997.mjs
...
__export(schema_exports, {
  commissionPayments: /* @__PURE__ */ __name(() => commissionPayments, "commissionPayments"),
  drivers: /* @__PURE__ */ __name(() => drivers, "drivers"),
  promotions: /* @__PURE__ */ __name(() => promotions, "promotions"),
  ratings: /* @__PURE__ */ __name(() => ratings, "ratings"),
  rides: /* @__PURE__ */ __name(() => rides, "rides"),
...
var drivers = sqliteTable("drivers", {
...
var commissionPayments = sqliteTable("commission_payments", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
...
var ratings = sqliteTable("ratings", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
...
var getSecret = /* @__PURE__ */ __name2((c) => c.env.JWT_SECRET || "dev-secret-keep-it-safe", "getSecret");
app.get("/health", (c) => c.json({ status: "ok", time: (/* @__PURE__ */ new Date()).toISOString() }));
...
app.get("/routing/route", async (c) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return c.json({ error: `Routing Error (${response.status}): ${errText.substring(0, 100)}` }, response.status);
    }
    const data = await response.json();
    return c.json(data);
  } catch (error) {
...
app.post("/auth/signup", async (c) => {
  ...
  try {
    ...
    const payload = { id: user.id, email: user.email, exp: Math.floor(Date.now() / 1e3) + 60 * 60 * 24 };
...
var authGuard = /* @__PURE__ */ __name2((c, next) => jwt({ secret: getSecret(c), alg: "HS256" })(c, next), "authGuard");
...
app.get("/rides/my", authGuard, async (c) => {
  const payload = c.get("jwtPayload");
  const db = drizzle(c.env.DB, { schema: schema_exports });
  const rides2 = await db.query.rides.findMany({
    where: eq(rides.passengerId, payload.id),
    orderBy: [desc(rides.createdAt)],
    limit: 20
  });
  return c.json(rides2);
});
app.get("/rides/my-active", authGuard, async (c) => {
  ...
  const activeRide = await db.query.rides.findFirst({
    where: and(
      eq(rides.passengerId, payload.id),
...
app.post("/rides/:id/cancel", authGuard, async (c) => {
  ...
  }).where(and(eq(rides.id, rideId), eq(rides.passengerId, payload.id)));
  return c.json({ success: true });
});
app.get("/driver/setup", authGuard, async (c) => {
  const payload = c.get("jwtPayload");
  const db = drizzle(c.env.DB, { schema: schema_exports });
  const driver = await db.query.drivers.findFirst({ where: eq(drivers.id, payload.id) });
...
app.post("/rides/:id/status", authGuard, async (c) => {
  ...
  const db = drizzle(c.env.DB, { schema: schema_exports });
  const update = { status };
  if (status === "in_progress") update.startedAt = (/* @__PURE__ */ new Date()).toISOString();
  if (status === "completed") {
    update.completedAt = (/* @__PURE__ */ new Date()).toISOString();
    const ride = await db.query.rides.findFirst({ where: eq(rides.id, rideId) });
    if (ride && ride.driverId) {
      const driver = await db.query.drivers.findFirst({ where: eq(drivers.id, ride.driverId) });
      if (driver) {
...
app.post("/driver/location", authGuard, async (c) => {
  ...
  await db.update(drivers).set({
    ...
    lastLocationUpdate: (/* @__PURE__ */ new Date()).toISOString(),
    isActive: true
  }).where(eq(drivers.id, payload.id));
  return c.json({ success: true });
});
app.get("/drivers/nearby", async (c) => {
  ...
  const drivers2 = activeDrivers.map((d) => ({
    id: d.id,
    position: [d.currentLatitude || lat + (Math.random() - 0.5) * 0.01, d.currentLongitude || lng + (Math.random() - 0.5) * 0.01],
    type: d.vehicleType === "motorcycle" ? "moto" : "taxi"
  }));
  return c.json({ drivers: drivers2 });
});
app.post("/verify-identity", authGuard, async (c) => {
  const { type } = await c.req.json();
  const payload = c.get("jwtPayload");
  const db = drizzle(c.env.DB, { schema: schema_exports });
  await db.update(users).set({ updatedAt: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(users.id, payload.id));
  if (type === "driver") {
    await db.update(drivers).set({ isVerified: true, isActive: true }).where(eq(drivers.id, payload.id));
  }
  const user = await db.query.users.findFirst({ where: eq(users.id, payload.id) });
...
app.post("/payments/webhook", async (c) => {
  ...
  if (body.type === "payment" && body.data?.id) {
    ...
    if (payment.status === "approved") {
      ...
      if (preferenceId) {
        await db.update(commissionPayments).set({
          status: "approved",
          mercadopagoPaymentId: paymentId.toString(),
          paidAt: (/* @__PURE__ */ new Date()).toISOString()
        }).where(eq(commissionPayments.mercadopagoPreferenceId, preferenceId));
      }
    }
  }
  return c.text("OK");
...
// .wrangler/tmp/bundle-TrhZkJ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TrhZkJ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.19429245638868997.js.map
